  ...
  const createEvent = (e) => {
    ...
    const find =
      ...
      busyTime.busyTime.find(
        (element) =>
          element.date === start.toDateString() &&
          element.hour === start.getHours() &&
          element.minute === start.getMinutes()
      );
      ...
      if (!optionView && !inPersonView && !onlineView && !formView) {
        if (view === "month") {
          ...
          if (stringArray[4] === "00:00:00") date = { ...date, hour: "" };
            ...
            date = { ...date, hour: stringArray[4] };
            setHour(stringArray[4].split(":").slice(0, 2).join(":"));
          }

          const startDate = `${date.year}-${numberMonth[date.month]}-${
            date.number
          }-${date.hour}`;
  ...
  const validate = (input, newStart, endClass, startClass, today) => {
    ...
    if (newStart) {
      ...
      date = calendar.find((e) => {
        ...
        return dateOutOfTime === endTime
          ? timeOfDateChange >= startClass
            ? timeOfDateChange <= endClass
              ? true
              : false
            : false
          : false;
    ...
    if ((today && !date) || (today && !newStart)) {
      date = busyTime.busyTime.find((e) => {
        return today.toDateString() === e.date
          ? e.hour + ":" + e.minute >= startClass
            ? e.hour + ":" + e.minute <= endClass
              ? true
              : false
            : false
          : false;
  ...
  return (
    <div className={style.divContainer}>
        ...
        <div className={style.formContainer}>
          <div className={style.closeButtonContainer}>
            <button
              className={style.closeButton}
              onClick={() => {
                setFormView(false), setWarninSing(false);
              }}
            >
              X
            </button>
          ...
          <div className={style.containerWithoutCloseButton}>
            ...
            <div className={style.inputTimeContainer}>
              <div className={style.inputContainer}>
                <div>
                  <label htmlFor="start">{`${days[dateGuard.day]} ${
                    dateGuard.number
                  } ${months[dateGuard.month]} ${hour}`}</label>
                </div>
                {/* <div>
      ...
      <input className={style.input} value={hour} onChange={(e) => {completedForm(e.target)}} type='time'  name='endDate' />
              ...
              {warningSign && (
                <p className={style.warningSignP}>
                  Horario reservado, cambiar horario por favor
                </p>
              )}
            </div>
            <div className={style.inputContainer}>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    completedForm(e.target);
                  }}
                  type="text"
                  name="clientName"
                />
                {errors.clientName && (
                  <p className={style.warningSignP}>{errors.clientName}</p>
                )}
              </div>
              <div>
                ...
                {errors.clientLastname && (
                  <p className={style.warningSignP}>{errors.clientLastname}</p>
                )}
              </div>
            </div>
            <div className={style.inputContainer}>
              <div>
                ...
                />
                {errors.email && (
                  <p className={style.warningSignP}>{errors.email}</p>
                )}
              </div>
              <div>
                ...
                />
                {errors.phone && (
                  <p className={style.warningSignP}>{errors.phone}</p>
                )}
              </div>
            </div>
